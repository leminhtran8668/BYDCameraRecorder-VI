...
**Nên cài bằng ADB** (Package Installer trên màn hình xe BYD thường báo lỗi phân tích gói / cú pháp):

```bash
adb connect <IP_XE>:5555
adb install -r build/byd-dashcam-debug.apk
```

Cấp quyền sau khi cài:
...
### Ghi hình tự động khi khởi động lại không hoạt động

App tự lưu trạng thái "đang ghi" và sẽ tự bật lại khi service khởi động lại
(mở app, hoặc xe thực sự reboot Android — gửi `BOOT_COMPLETED`). Lần mở app
đầu tiên, app sẽ xin quyền **bỏ qua tối ưu hóa pin** (system dialog) — nên
đồng ý, nếu không hệ thống có thể tự kill tiến trình chạy nền giữa các
chuyến đi và bạn phải bấm ghi lại thủ công.

Nhiều đầu màn hình BYD/DiLink **không thực sự reboot Android** mỗi lần bật
máy (chỉ vào chế độ chờ), nên `BOOT_COMPLETED` không phải lúc nào cũng bắn
ra. Nếu ROM xe bạn có mục **Cài đặt → Ứng dụng → Quản lý tự khởi động /
Auto-start Management**, hãy bật quyền tự khởi động cho app này — đây là
danh sách trắng riêng của ROM, không liên quan gì đến tối ưu hóa pin chuẩn
của Android.

### Lưu ý build / lỗi parse trên xe

- `build.sh` đã chỉnh cho DiLink:
  - Nhúng DEX bằng `zip -0` (không nén), bỏ `zip -u`
  - Chữ ký **v1 + v2** (tắt v3)
  - d8 dùng `--min-api 23`
- Trước khi build (nếu chưa có `assets/phone/`):  
  `npm --prefix phone-ui install && npm --prefix phone-ui run build`
- Nếu vẫn lỗi: gửi nguyên dòng lỗi `adb install` (ví dụ `INSTALL_PARSE_FAILED_...`)
