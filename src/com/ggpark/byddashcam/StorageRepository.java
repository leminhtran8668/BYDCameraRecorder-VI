...
public final class StorageRepository {
    ...
    public List<StorageVolume> getVolumes() {
        ...
        for (int index = 0; index < directories.length; index++) {
            ...
            // Some car firmwares refuse to create the nested
            // Android/data/<pkg>/files path on removable media even though
            // the card itself is mounted read-write. In that case fall back
            // to writing directly under the volume's public root, which the
            // legacy WRITE_EXTERNAL_STORAGE permission (targetSdkVersion 28,
            // pre-scoped-storage) still allows.
            File usableRoot = directory;
            if (!directory.exists() && !directory.mkdirs()) {
                File volumeRoot = publicVolumeRootOf(directory);
                if (volumeRoot != null && volumeRoot.exists() && volumeRoot.canWrite()) {
                    Log.w(
                            TAG,
                            "Cannot create app-private storage dir: "
                                    + directory
                                    + " — falling back to volume root: "
                                    + volumeRoot);
                    usableRoot = volumeRoot;
                } else {
            ...
            String pathStr = usableRoot.getAbsolutePath();
            boolean looksLikeSd =
                    removable
            ...
            long freeMb = 0L;
            try {
                StatFs stat = new StatFs(usableRoot.getAbsolutePath());
            ...
            String label = String.format(
                    ...
                    typeLabel,
                    freeMb);
            result.add(new StorageVolume(index, label, usableRoot));
        }
        return result;
    }

    /**
     * Walks a per-app external-files directory
     * (".../Android/data/&lt;pkg&gt;/files") back up to the volume's public
     * root (e.g. "/storage/5CCB-7B77"), for use when the nested
     * Android/data path itself cannot be created. Returns null if the
     * directory doesn't follow the expected shape.
     */
    private File publicVolumeRootOf(File appFilesDir) {
        File current = appFilesDir;
